...
The CV link points to `paper/resume.pdf`. That PDF is generated by GitHub
Actions from `resume/main.tex` during deployment and is intentionally ignored in
the repository. For local CV previews only, run `make cv`.
